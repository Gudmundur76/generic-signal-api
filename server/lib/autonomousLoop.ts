...
import { scoreResistanceProfile, getKeyMutationSummary } from "./resistAgent";
import { computeArbitrageOpportunity } from "./patentArbitrage";
  ...
  try {
    ...
    for (const signal of signals) {
      ...
      try {
        ...
        // STEP 3c: RESISTANCE SCORING — score top candidate against known resistance mutations
        // Only applies to HIV protease targets; for cardiovascular targets this is informational only.
        const topFitness = evolveResult.topCandidates[0]?.fitness ?? 0;
        // Convert fitness (0–100) to pIC50 estimate (6.0–9.0 range)
        const estimatedPIC50 = 6.0 + (topFitness / 100) * 3.0;
        const resistanceProfile = scoreResistanceProfile(estimatedPIC50, "generic");
        const keyMutations = getKeyMutationSummary(resistanceProfile);
        console.log(
          `[autonomous] ${signal.gene} resistance profile: ` +
          `robustness=${resistanceProfile.robustnessScore}, ` +
          `recommendation=${resistanceProfile.recommendation}, ` +
          `V82A=${keyMutations.V82A.passes ? 'pass' : 'fail'}, ` +
          `I84V=${keyMutations.I84V.passes ? 'pass' : 'fail'}, ` +
          `L90M=${keyMutations.L90M.passes ? 'pass' : 'fail'}`
        );

        // STEP 3d: PATENT ARBITRAGE — identify best filing jurisdiction
        const landscape = await fetchPatentLandscape(signal.gene);
        // Build jurisdiction coverage from the patent landscape data
        const usCount = landscape.patents.filter(p => p.jurisdiction === "US").length;
        const epCount = landscape.patents.filter(p => p.jurisdiction === "EP").length;
        const woCount = landscape.patents.filter(p => p.jurisdiction === "WO").length;
        const jurisdictionCoverage: import('./patentArbitrage').PatentCoverage[] = [
          { jurisdiction: "US" as const, coverageScore: usCount > 0 ? 0.9 : 0.1, patentCount: usCount, latestExpiry: landscape.nearestExpiration },
          { jurisdiction: "EP" as const, coverageScore: epCount > 0 ? 0.8 : 0.1, patentCount: epCount, latestExpiry: undefined },
          { jurisdiction: "JP" as const, coverageScore: 0.2, patentCount: 0, latestExpiry: undefined },
          { jurisdiction: "CN" as const, coverageScore: 0.2, patentCount: 0, latestExpiry: undefined },
          { jurisdiction: "WO" as const, coverageScore: woCount > 0 ? 0.7 : 0.1, patentCount: woCount, latestExpiry: undefined },
          { jurisdiction: "CA" as const, coverageScore: 0.15, patentCount: 0, latestExpiry: undefined },
          { jurisdiction: "AU" as const, coverageScore: 0.15, patentCount: 0, latestExpiry: undefined },
          { jurisdiction: "IN" as const, coverageScore: 0.1, patentCount: 0, latestExpiry: undefined },
        ];
        const arbitrage = computeArbitrageOpportunity(
          `CAND-${signal.gene}-${Date.now()}`,
          signal.gene,
          jurisdictionCoverage,
        );
        console.log(
          `[autonomous] ${signal.gene} patent arbitrage: ` +
          `ipGapScore=${arbitrage.overallIpGapScore}, ` +
          `recommendation=${arbitrage.recommendation}, ` +
          `highOpportunity=${arbitrage.highOpportunityJurisdictions.join(',') || 'none'}, ` +
          `filingWindow=${arbitrage.estimatedFilingWindow}`
        );
        ...
        // Pass evidence PDF URL if available (generated by asi-evolve and uploaded to S3)
        const evidencePdfUrl = (evolveResult as DnaEvolveResult & { evidencePdfUrl?: string }).evidencePdfUrl ?? undefined;
        await deliverToPartner(partner.id, signal, evolveResult, resistanceProfile, arbitrage, evidencePdfUrl);
...
async function deliverToPartner(
  ...
  resistanceProfile?: import('./resistAgent').ResistanceProfile,
  arbitrage?: import('./patentArbitrage').ArbitrageOpportunity,
  ...
  await fetch(`${AutonomousConfig.endpoints.genericSignal}/api/trpc/partners.recordDelivery`, {
    ...
    headers: {
      ...
      ...(ENV.loopServiceKey ? { "x-service-key": ENV.loopServiceKey } : {}),
    },
    body: JSON.stringify({
      candidateId,
      partnerId,
      ...
      // Resistance profile
      resistanceRobustness: resistanceProfile?.robustnessScore ?? null,
      resistanceRecommendation: resistanceProfile?.recommendation ?? null,
      resistanceOverallPass: resistanceProfile?.overallPass ?? null,
      // Patent arbitrage
      ipGapScore: arbitrage?.overallIpGapScore ?? null,
      ipRecommendation: arbitrage?.recommendation ?? null,
      highOpportunityJurisdictions: arbitrage?.highOpportunityJurisdictions ?? [],
      estimatedFilingWindow: arbitrage?.estimatedFilingWindow ?? null,
      // Evidence PDF — URL or file path for the partner to download the full evidence dossier
      evidencePdfUrl: evidencePdfUrl ?? null,
    }),
